...
window.Zepto = Zepto, void 0 === window.$ && (window.$ = Zepto),
	...
	function(e) {
		function t(e, t) {
			var n = this.os = {},
				...
				x = !w && e.match(/(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)/),
				T = x || e.match(/Version\/([\d.]+)([^S](Safari)|[^M]*(Mobile)[^S]*(Safari))/);
	...
	function(e, t) {
		...
		e.each(v, function(e, n) {
			...
			var g, E, S, x = {},
				...
				k = !1;
			if(i === t && (i = e.fx.speeds._default / 1e3), m === t && (m = 0), e.fx.off && (i = 0), "string" == typeof r) x[l] = r, x[c] = i + "s", x[p] = m + "s", x[h] = d || "linear", C = e.fx.animationEnd;
			else {
				E = [];
	...
	function(e) {
		...
		function pt(e, t) {
			...
			if(m.length > 0)
				for(n in m) r = m[n], i = t[r], typeof i != "undefined" && (e[r] = i);
			return e
		}
		...
		function un(e, n, r) {
			var i = t.duration(e).abs(),
				...
				l = s(i.as("M")),
				c = s(i.as("y")),
				h = o < K.s && ["s", o] || u === 1 && ["m"] || u < K.m && ["mm", u] || a === 1 && ["h"] || a < K.h && ["hh", a] || f === 1 && ["d"] || f < K.d && ["dd", f] || l === 1 && ["M"] || l < K.M && ["MM", l] || c === 1 && ["y"] || ["yy", c];
		...
		function ln(n) {
			...
			return n._locale = n._locale || t.localeData(n._l), r === null || i === e && r === "" ? t.invalid({
				nullInput: !0
			}) : (typeof r == "string" && (n._i = r = n._locale.preparse(r)), t.isMoment(r) ? new lt(r, !0) : (i ? wt(i) ? Gt(n) : Jt(n) : tn(n), s = new lt(n), s._nextDay && (s.add(1, "d"), s._nextDay = e), s))
		}
		...
		var t, n = "2.8.4",
			...
			v = {},
			m = [],
			g = typeof module != "undefined" && module && module.exports,
			y = /^\/?Date\((\-?\d+)/i,
			b = /(\-)?(?:(\d*)\.)?(\d+)\:(\d+)(?:\:(\d+)\.?(\d{3})?)?/,
			w = /^(-)?P(?:(?:([0-9,.]*)Y)?(?:([0-9,.]*)M)?(?:([0-9,.]*)D)?(?:T(?:([0-9,.]*)H)?(?:([0-9,.]*)M)?(?:([0-9,.]*)S)?)?|([0-9,.]*)W)$/,
			E = /(\[[^\[]*\])|(\\)?(Mo|MM?M?M?|Do|DDDo|DD?D?D?|ddd?d?|do?|w[o|w]?|W[o|W]?|Q|YYYYYY|YYYYY|YYYY|YY|gg(ggg?)?|GG(GGG?)?|e|E|a|A|hh?|HH?|mm?|ss?|S{1,4}|x|X|zz?|ZZ?|.)/g,
			...
			z = /([\+\-]|\d\d)/gi,
			W = "Date|Hours|Minutes|Seconds|Milliseconds".split("|"),
			X = {
				Milliseconds: 1,
				Seconds: 1e3,
				Minutes: 6e4,
			...
			Q = "DDD w W M D d".split(" "),
			G = "M D H h m s w W".split(" "),
			Y = {
				M: function() {
					return this.month() + 1
				},
			...
			Z = {},
			et = ["months", "monthsShort", "weekdays", "weekdaysShort", "weekdaysMin"];
		...
		}, t.version = n, t.defaultFormat = q, t.ISO_8601 = function() {}, t.momentProperties = m, t.updateOffset = function() {}, t.relativeTimeThreshold = function(t, n) {
		...
		t.duration.fn.asMilliseconds = function() {
			...
			ordinal: function(e) {
				var t = e % 10,
					n = Ct(e % 100 / 10) === 1 ? "th" : t === 1 ? "st" : t === 2 ? "nd" : t === 3 ? "rd" : "th";
				return e + n
			}
		...
		var t = "http://app.lehumall.com/html5/app/",
			n = "http://app.lehumall.com/",
			r = e("#version").attr("data-version") || Date.now(),
			i = {
				index: t + "?t=" + r,
		...
		var u = n.Construct.extend({
			...
			request: function(t, r, i) {
				...
				return e.ajax({
					url: this.url,
					type: r || "get",
					contentType: r == "get" ? "text/x-json;charset=UTF-8" : "application/json; charset=UTF-8",
					data: t,
					success: function(e) {
						var t;
						try {
		...
		return t.route.ready(), window.onerror = function(e, t, n) {}, {
			...
			isMobile: {
				...
				APP: function() {
					var e = s.setting.is_app || i.get("IS_APP"),
			...
			tip: function(t, n) {
				var r = e('<div class="dialog-cart" style="z-index:9999;"><div class="dialog-cart-inner" style="padding:10px 20px;"><p>' + t + "</p></div></div>");
				...
				var r = this,
					s = {
						NONE: function(e, t) {
							var r = "sfhaitao.xyz!";
							return n.extend(e, {
								_sig: sf.util.encrypt(e, r)
							})
						},
						USERLOGIN: function(e, t) {
							var s = i.get("csrfToken");
							if(s) return n.extend(e, {
								_sig: r.encrypt(e, s)
							});
							if(!t) return e;
		...
		var n = e.Control.extend({
			init: function() {
				this.initRem(), this.showDocument()
			},
		...
		var o = t.deparam(window.location.search.substr(1)),
			u = function(e) {
			...
			a = function() {
				...
				if(s.isMobile.Android() && window.JSInterface) e.SERVER_URL = JSInterface.getServerUrl(), e.IMAGE_URL = JSInterface.getImageUrl();
				else {
			...
			f = function(e) {
				s.isMobile.Android() ? (e = JSON.stringify(e), JSInterface.nativeFunction(e)) : s.isMobile.iOS() && u(function(t) {
					t.registerHandler(e.funName, function(t, n) {
						n(e.params)
		...
		var i = function() {},
			s = n.Control.extend({
				...
				register: function(e, t) {
					var n = Date.now(),
						r = "sfht_" + n;
					return this.troops[r] = {
						success: e || i,
						error: t || i
					}, r
				},
				run: function(t, n, i, s, o) {
					var u = this.register(s, o),
						a, f = JSON.stringify(i);
					try {
		...
		var i = ["web", "wechat", "alipay", "localapp", "onlineapp", "app"],
			s = "web";
